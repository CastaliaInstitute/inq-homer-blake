...
| Repository validators | **B-** | Structural, source, density, meter, EPUB, provenance, and geometry checks are executable; the manuscript duplication preflight now checks both repeated four-line blocks and accidental adjacent duplicate verse across all 48 openings and their collation authorities. The BookVault source-hash validator fails closed when a candidate PDF no longer matches its manuscript source. | Rerun current-branch CI and final-art/final-PDF checks after every text or art lock, and close all human gates. Automated green status is necessary but not sufficient. |
