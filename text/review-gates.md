...
No book is “final” because a draft reads smoothly. Each of the forty-eight
books must pass every gate below, with a named reviewer and date recorded in
the book's decision log.
...
Use exactly one status in each book record:

- `outline` — scene and source range mapped; no verse approved;
- `draft` — working English exists, but one or more literary gates remain;
- `review` — all verse exists and is under line-level review;
- `approved` — all six gates signed off;
- `laid-out` — approved text placed in the production file;
