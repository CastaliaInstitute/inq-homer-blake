# Odyssey Book 21 — read-aloud and fidelity review

**Review status:** translator self-review, revision pass; independent reader still required
**Revision date:** 2026-09-03

## Findings

The manuscript now preserves the bow’s guest-friendship provenance: Iphitus met
Odysseus in Messenia, Heracles violated Zeus’s guest-law by killing Iphitus and
taking the horses, and Odysseus kept the bow as a memorial rather than carrying
it to Troy. Penelope’s storeroom, key, roaring doors, bow speech, and twelve axes
remain the public frame of the contest.

Telemachus’s role is restored: he sets the axes, tries three times to string the
bow, and is stopped by his father’s nod on the fourth attempt. The private
recognition of Eumaeus and Philoetius then precedes the public request for the
beggar to try, so the arrow’s passage is both a contest result and the signal to
begin the reckoning.

The postponed contest, Antinous’s threats, Penelope’s defense, Telemachus’s
assertion of authority, the door-locking instructions, and the bow’s stringing
were expanded into verse in this pass. The revised cadence moves from delay to
confinement to release; it remains working evidence pending independent review.

## Beauty and cadence

The bow should enter as a relic before it becomes a weapon. Penelope’s formal
challenge has the measured ceremony of a legal test; the failed attempts tighten
the hall’s breath. The scar recognition outside should sound intimate and urgent,
then return to the public room with the long, almost effortless stringing and the
single arrow’s clean passage through the axes.

## Open decisions

- Independently verify Iphitus’s horses, the axes’ arrangement, and the exact
  order of the private recognition and public contest.
- Check the smoke explanation, door-locking instructions, and remaining weapons
  against the full Greek.
- Commission an independent read-aloud and omissions/additions report.

**Conclusion:** provenance and contest mechanics are restored; not approved for
layout or final publication.
